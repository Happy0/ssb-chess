  ...
  return {
    ...
    unseenNotifications: () => {
      ...
      return computed([unseen], (unseenMessages) => {
        ...
        const msgIndex = unseenMessages.findIndex(
          entry => entry.msg.value.content.root === currentGame,
        );

        if (currentGame && (msgIndex !== -1)) {
          unseenMessages.splice(msgIndex, 1);

          // If it's the only one that was in the list, make sure the user won't
          // see it when they open the app again
